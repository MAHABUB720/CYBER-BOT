module.exports.config = {
    ...
    var c, u = a.body;
    if (c = u, isNaN(c) || (c < 1 || c > 6)) return e.sendMessage("Error! An error occurred. Please try again later.", a.threadID, a.messageID);
    ...
    try {
        ...
        return s.writeFileSync(path1, Buffer.from(i, "utf-8")), e.unsendMessage(t.messageID), s.statSync(__dirname + "/cache/1.mp3").size > 26e6 ? e.sendMessage("The file could not be sent because it is larger than 25MB.", a.threadID, (() => l(__dirname + "/cache/1.mp3")), a.messageID) : e.sendMessage({
            body: `» ${y}`,
    ...
    var u = ["AIzaSyD8w-hBPzqPee4PJVtSFaWsx3__BPWcDxg", "AIzaSyAyjwkjc0w61LpOErHY_vFo6Di5LEyfLK0", "AIzaSyBY5jfFyaTNtiTSBNCvmyJKpMIGlpCSB4w", "AIzaSyCYCg9qpFmJJsEcr61ZLV5KsmgT1RE5aI4"];
    ...
    if (0 == t.join(" ").indexOf("https://")) {
        ...
        var b = (await n.request(f)).data,
            v = b.title;
        if ("fail" == b.status) return e.sendMessage("❗This file could not be sent.", a.threadID);
        ...
        return s.writeFileSync(path1, Buffer.from(i, "utf-8")), s.statSync(__dirname + "/cache/1.mp3").size > 26e6 ? e.sendMessage("Sorry, Files above 25mb cannot be sent.", a.threadID, (() => h(__dirname + "/cache/1.mp3")), a.messageID) : e.sendMessage({
            body: `♪${v}`,
            attachment: s.createReadStream(__dirname + "/cache/1.mp3")
        }, a.threadID, (() => s.unlinkSync(__dirname + "/cache/1.mp3")), a.messageID)
    }
    try {
        ...
        var j = `Search Success! There are ${w.length} songs.\n\n${_}Reply (reply) by Order of number you want to choose.`;
